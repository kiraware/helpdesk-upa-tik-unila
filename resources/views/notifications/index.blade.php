        ...
        <div class="flex items-center justify-between">
            ...
            @if (auth()->user()->unreadNotifications->count() > 0)
                <form action="{{ route('notifications.markAll') }}" method="POST">
                    @csrf
                    <button type="submit"
                        class="px-4 py-2 bg-white dark:bg-slate-800 border border-border-light dark:border-slate-700 rounded-lg text-sm shadow-sm hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">
                        Tandai Semua Dibaca
                    </button>
                </form>
            @endif
            ...
            <ul class="divide-y divide-border-light dark:divide-border-dark">
                @forelse($notifications as $notification)
                    <li
                        ...
                        <a href="{{ route('notifications.read', $notification->id) }}" class="block p-5">
                            <div class="flex items-start gap-4">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between gap-2 mb-1">
                                        <p class="text-sm font-semibold text-text-light dark:text-slate-200">
                                            {{ $notification->data['title'] }}
                                        </p>
                                        <span class="text-xs text-muted-light dark:text-slate-500 whitespace-nowrap">
                                            {{ $notification->created_at->translatedFormat('d M Y, H:i') }}
                                        </span>
                                    </div>
                                    <p class="text-sm text-muted-light dark:text-slate-400">
